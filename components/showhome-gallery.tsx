...
function MovingStrip({ reverse = false }: { reverse?: boolean }) {
  ...
  return <div className="relative overflow-hidden">
    <div ref={trackRef} className="flex w-max gap-3 will-change-transform">
      {[...showhomeImages, ...showhomeImages].map((src, index) => <ImageLightbox key={`${src}-${index}`} size="floor" className="h-[150px] w-max shrink-0 transition-transform duration-500 hover:scale-[1.025] max-md:h-[105px]" src={src} alt="Không gian nhà mẫu BCONS Central Park"><img src={src} alt="Không gian nhà mẫu BCONS Central Park" className="block h-[150px] w-auto max-w-none object-contain max-md:h-[105px]" /></ImageLightbox>)}
    </div>
